...
const Home = ({ foodAreas, order, resetOrderSettings }) => {
  resetOrderSettings();
  return (
    <React.Fragment>
      <ul className="Home">
            ...
            <div className="Home__fixed-content">
              <header className="Home__header">
                ...
                <Link to="/" className="Home__change-tz">
                  <img className="Home__edit" alt="change-profile" src={edit} />
                </Link>
              </header>
              <ul className="Home__tabs">
                <li className="Home__tab Home__tab_active">Еда</li>
            ...
            <ul className="Home__items">
              {area.items.map((item) => (
                <li className="Home__item" key={item.id}>
                  <Link className="Home__food-link" to={item.link}>
                    <img
                      alt={item.name}
                      className="Home__image"
                      src={item.image}
                    />
                    <h3 className="Home__food-name">{item.name}</h3>
